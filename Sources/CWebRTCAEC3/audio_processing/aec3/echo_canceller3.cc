...
void RetrieveFieldTrialValue(const FieldTrialsView& field_trials,
  ...
  float field_trial_value_float = *value_to_update;
  FieldTrialParameter<float> field_trial_param(/*key=*/"", field_trial_value_float);
  ...
  if (field_trial_value >= min && field_trial_value <= max &&
    ...
    RTC_LOG_V(rtc::LS_INFO) << "Key " << trial_name
...
EchoCanceller3Config AdjustConfig(const EchoCanceller3Config& config,
  ...
  FieldTrialParameter<float> nearend_reverb_default_len(
      "default_len", adjusted_cfg.ep_strength.default_len);
  FieldTrialParameter<float> nearend_reverb_nearend_len(
